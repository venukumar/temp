	...
	public void onClick(View v) {
		...
		case R.id.sign_in_button:
		    if (!mPlusClient.isConnected()) {
		        ...
		        Toast.makeText(this, "Already logged in to Google...", Toast.LENGTH_SHORT).show();
		    break;
...
//                mPlusClient.connect();
            	...
            	Toast.makeText(this, "Already logged out from Google", Toast.LENGTH_SHORT).show();
			break;
		case R.id.button_revoke:
			if (!mPlusClient.isConnected()) {
				// Need to be connected in order to revoke access
				mPlusClient.connect();
		        Toast.makeText(this, "Need to be logged in to disconnect App", Toast.LENGTH_SHORT).show();
				break;
			}
	...
	protected void onActivityResult(int requestCode, int responseCode, Intent intent) {
		Log.d(TAG, "Activity result for request: " + requestCode + " with response: " + responseCode);
	...
	public void onPersonLoaded(ConnectionResult status, Person arg1) {
	    if (status.getErrorCode() == ConnectionResult.SUCCESS) {
	    	...
	    	new DownloadImageTask().execute(arg1.getImage().getUrl(),  
	    			getResources().getString(R.string.config_user_profile_picture));
    	    ...
    	    editor.putString(r.getString(R.string.config_user_name), mPerson.getDisplayName());
    	    editor.putString(r.getString(R.string.config_user_location), mPerson.getCurrentLocation());
	...
	private class DownloadImageTask extends AsyncTask<String, Integer, Bitmap> {
        ...
        protected Bitmap doInBackground(String... params) {
        	// Kind of inefficient way to download an image. Need to just save the file as it comes...
    	    ...
    	    // Save bitmap to file
    	    Log.d(TAG, "Saving user profile to " + getFilesDir()+File.separator + file);

    	    try {
                FileOutputStream out = new FileOutputStream(getFilesDir()+File.separator + file);

                bitmap.compress(Bitmap.CompressFormat.JPEG, 90, out);
            } catch (Exception e) {
                e.printStackTrace();
        	    Log.d(TAG, "Could not save user profile to " + getResources().getString(R.string.config_user_profile_picture));
    ...
    private class InitAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup collection, int position) {
        	...
        	case 1:
        		...
        		ImageButton b = (ImageButton) v.findViewById(R.id.button_disconnect);
