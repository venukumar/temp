...
public class ContactsService extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.contact_host_main);
    ...
    /**
     * Local implementation of the ContactAddressBookInterface
     */
    private class AllJoynContactService implements ContactAddressBookInterface, BusObject {
         ...
         */
        public ContactNameId[] getAllContactNames() {
             ...
             */
            int ContactNameIdx = cursor.getColumnIndexOrThrow(ContactsContract.Contacts.DISPLAY_NAME);
            int idIdx   = cursor.getColumnIndexOrThrow(ContactsContract.Contacts._ID);

            ContactNameId[] result;
            int count = cursor.getCount();
            if (count > 0) {
                result = new ContactNameId[cursor.getCount()];
                if (cursor.moveToFirst()) {
                    do {
                        result[cursor.getPosition()] = new ContactNameId();
                        result[cursor.getPosition()].displayName = cursor.getString(ContactNameIdx);
                 ...
                 */
                result = new ContactNameId[1];
                result[0] = new ContactNameId();
