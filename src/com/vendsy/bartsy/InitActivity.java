...
public class InitActivity extends SherlockFragmentActivity implements
		...
		OnPersonLoadedListener, OnClickListener, LoginDialogListener {
...
//		findViewById(R.id.view_init_facebook).setOnClickListener(this);
	...
	void signUpListeners() {
		...
		((TextView) findViewById(R.id.view_init_google))
				.setText("Sign up with Google");
		((TextView) findViewById(R.id.view_init_facebook))
				.setText("Sign up with Facebook");
	}
...
//		 Intent fbIntent = new Intent(InitActivity.this,
...
//		 break;
	...
	/**
	 * This function is called by the user login dialog. The dialog provides a
	 * username/password and this function gets the rest of the information for
	 * the user profile or displays a Toast if the username/password are
	 * incorrect
	 */
	...
	public void onDialogPositiveClick(LoginDialogFragment dialog) {
		...
		new Thread() {
			public void run() {
				...
				// Logged in successfully and obtained user information. Save
				// it.
				...
				mHandler.post(new Runnable() {
					...
					public void run() {
						...
						Toast.makeText(mActivity,
								"Logged in as " + user.getBartsyLogin(),
								Toast.LENGTH_SHORT).show();

						// We got an existing user. Start profile edit activity
						// using this user and the input
	...
	protected void onActivityResult(int requestCode, int responseCode,
		...
		Log.v(TAG, "Activity result for request: " + requestCode
				...
				if (mConnectionProgressDialog.isShowing())
				 ...
				 loginButton.setUserInfoChangedCallback(new LoginButton.UserInfoChangedCallback() {
			            ...
			            public void onUserInfoFetched(GraphUser user) {
			            	if(user!=null){
//			            	mApp.mUserProfileActivityInput = new UserProfile(user);
			...
			default:
				...
				Toast.makeText(this, "Could not download Facebook information",
						Toast.LENGTH_SHORT).show();
				if (mConnectionProgressDialog.isShowing())
					mConnectionProgressDialog.dismiss();
				break;
		...
		case REQUEST_CODE_USER_PROFILE:
			switch (responseCode) {
			case RESULT_OK:
				// We got a response from the user profile activity. Process the
				// user profile and start
				// the right activity if successful
				Log.v(TAG, "Profile saved - process results");
				Intent action = new Intent().setClass(InitActivity.this,
						MainActivity.class);
	...
	public void onConnected(Bundle connectionHint) {
		...
		// Toast.makeText(this, "Connected as " + mAccountName,
		...
		if (status.getErrorCode() == ConnectionResult.SUCCESS) {
			...
			new Thread() {
				public void run() {
					...
					// User log-in syscall
					UserProfile profile = WebServices
							.getUserProfile(mApp, user);

					if (profile == null) {

						// Could not log in. We need to create a new profile
						// based on the information obtained.
						mHandler.post(new Runnable() {
							@Override
							public void run() {
								UserProfileActivity.setInput(mApp, user); // use
																			// the
																			// application
																			// as
																			// a
					...
					mHandler.post(new Runnable() {
						...
						public void run() {
							Toast.makeText(mActivity,
									"Logged in as " + user.getGoogleUsername(),
									Toast.LENGTH_SHORT).show();
							Intent intent = new Intent().setClass(
									InitActivity.this, MainActivity.class);
							intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
	...
	private class DownloadAndSaveUserProfileImageTask extends
